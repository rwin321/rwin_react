...
const initialState = {
  posts: [
    ...
    {
      id: 4,
      message: "To get succeed, first you must learn how to fail",
      likesCount: 13,
    },
...
const profileReducer = (
  ...
  switch (action.type) {
    ...
    case DELETE_POST: {
      let stateCopy = { ...state };
      stateCopy.posts = [...state.posts];
      stateCopy.posts = stateCopy.posts.filter(
...
export const setStatusProfile = (status: string): SetStatusType => ({
...
export const saveProfile =
  (profile: ProfileType) => async (dispatch: any, getState: any) => {
    ...
    if (response.data.resultCode === 0) {
      ...
      dispatch(
        stopSubmit("edit-profileData", { _error: response.data.messages[0] })
      );
